...
int main (int argc, char * argv[]){
	...
	for (int dt = 0 ; dt < nGen; dt++ ){
		...
		if ((patchArr[i][j] == 0) && (popArr[i][j] > 0 )) {
		...
		if (( checkEmpty(popArr, neighborPick[0],neighborPick[1], nDemes) == 1  ) ) {
		...
		for (int ne =0;ne<4;ne++){
			...
			if ((patchArr[i][j] == 1) &&( find(cellList.begin(), cellList.end(), neighbID) != cellList.end() )  &&  (checkEmpty(popArr, neighbCoor[ne][0] ,neighbCoor[ne][1], nDemes) == 0 ) ){
